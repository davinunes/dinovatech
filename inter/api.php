...
function criarLocationRecorrencia($config, $sslCert, $sslKey, $caInfo, $bearerToken)
{
    ...
    if ($error) {
        throw new Exception("cURL Error on POST /locrec: " . $error . " | HTTP Code: " . $httpCode);
    }
    if ($httpCode >= 400) {
        throw new Exception("API Error on POST /locrec: " . $response . " | HTTP Code: " . $httpCode);
    }

    $decoded = json_decode($response);
    if (!$decoded || empty($decoded->id)) {
        throw new Exception("Resposta inválida do Inter em POST /locrec: " . $response);
    }
