                        ...
                        <tbody class="text-gray-700 text-sm">
                            <?php if (!empty($contratos)): ?>
                                <?php foreach ($contratos as $contrato):
                                    ...
                                    $row_class = $is_expired ? 'bg-red-50' : 'hover:bg-gray-50';
                                    ?>
                                    <tr class="<?= $row_class ?> transition border-b border-gray-100 last:border-b-0">
            ...
            <div class="md:hidden space-y-4">
                <?php if (!empty($contratos)): ?>
                    <?php foreach ($contratos as $contrato):
                        ...
                        ?>
                        <div class="<?= $card_class ?> p-4 rounded-xl shadow-sm border">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    ...
                                    <?php if ($is_expired): ?>
                                        <span
                                            class="mt-1 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">
                                            Vencido
                                        </span>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="text-sm text-gray-600 mb-4 flex justify-between">
                                ...
                                <?php if (!empty($contrato['data_fim'])): ?>
                                    <span>Fim: <?= date('d/m/Y', strtotime($contrato['data_fim'])) ?></span>
                                <?php endif; ?>
                            </div>

                            <div class="pt-3 border-t border-gray-200/50 flex justify-end">
                                <a href="contrato_form.php?id=<?= $contrato['id_recorrencia'] ?>"
                                    class="w-full text-center bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 py-2 rounded-lg text-sm font-medium transition-colors">
                                    Editar Contrato
                                </a>
                            </div>
                        </div>
